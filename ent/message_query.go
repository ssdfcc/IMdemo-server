...
func (mq *MessageQuery) QueryRecipient() *UserQuery {
	query := (&UserClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withRecipient: mq.withRecipient.Clone(),
...
func (mq *MessageQuery) WithRecipient(opts ...func(*UserQuery)) *MessageQuery {
	query := (&UserClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withRecipient = query
	return mq
}
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withRecipient; query != nil {
		if err := mq.loadRecipient(ctx, query, nodes, nil,
			func(n *Message, e *User) { n.Edges.Recipient = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadRecipient(ctx context.Context, query *UserQuery, nodes []*Message, init func(*Message), assign func(*Message, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		fk := nodes[i].TargetID
...
func (mq *MessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withSender != nil {
			_spec.Node.AddColumnOnce(message.FieldFormID)
		}
		if mq.withRecipient != nil {
			_spec.Node.AddColumnOnce(message.FieldTargetID)
		}
	}
