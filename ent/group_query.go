...
type GroupQuery struct {
	...
	withGroupRelationGroup *GroupRelationQuery
	withTargetMsg          *GroupMsgQuery
...
func (gq *GroupQuery) QueryOwnerUser() *UserQuery {
...
// QueryGroupRelationGroup chains the current query on the "group_relation_group" edge.
func (gq *GroupQuery) QueryGroupRelationGroup() *GroupRelationQuery {
	query := (&GroupRelationClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) QueryTargetMsg() *GroupMsgQuery {
	query := (&GroupMsgClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withOwnerUser:          gq.withOwnerUser.Clone(),
		withGroupRelationGroup: gq.withGroupRelationGroup.Clone(),
		withTargetMsg:          gq.withTargetMsg.Clone(),
...
func (gq *GroupQuery) WithOwnerUser(opts ...func(*UserQuery)) *GroupQuery {
	query := (&UserClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withOwnerUser = query
	return gq
}

// WithGroupRelationGroup tells the query-builder to eager-load the nodes that are connected to
// the "group_relation_group" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithGroupRelationGroup(opts ...func(*GroupRelationQuery)) *GroupQuery {
	query := (&GroupRelationClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withGroupRelationGroup = query
	return gq
}
...
func (gq *GroupQuery) WithTargetMsg(opts ...func(*GroupMsgQuery)) *GroupQuery {
	query := (&GroupMsgClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withTargetMsg = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	var (
		...
		_spec       = gq.querySpec()
		loadedTypes = [3]bool{
			gq.withOwnerUser != nil,
			gq.withGroupRelationGroup != nil,
			gq.withTargetMsg != nil,
		}
	)
	...
	if query := gq.withOwnerUser; query != nil {
		if err := gq.loadOwnerUser(ctx, query, nodes, nil,
			func(n *Group, e *User) { n.Edges.OwnerUser = e }); err != nil {
			return nil, err
		}
	}
	if query := gq.withGroupRelationGroup; query != nil {
		if err := gq.loadGroupRelationGroup(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupRelationGroup = []*GroupRelation{} },
			func(n *Group, e *GroupRelation) { n.Edges.GroupRelationGroup = append(n.Edges.GroupRelationGroup, e) }); err != nil {
			return nil, err
		}
	}
	if query := gq.withTargetMsg; query != nil {
		if err := gq.loadTargetMsg(ctx, query, nodes,
			func(n *Group) { n.Edges.TargetMsg = []*GroupMsg{} },
			func(n *Group, e *GroupMsg) { n.Edges.TargetMsg = append(n.Edges.TargetMsg, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadOwnerUser(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
...
func (gq *GroupQuery) loadGroupRelationGroup(ctx context.Context, query *GroupRelationQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupRelation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(grouprelation.FieldGroupID)
	}
	query.Where(predicate.GroupRelation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.GroupRelationGroupColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (gq *GroupQuery) loadTargetMsg(ctx context.Context, query *GroupMsgQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupMsg)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupmsg.FieldTargetID)
	}
	query.Where(predicate.GroupMsg(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.TargetMsgColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (gq *GroupQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := gq.ctx.Fields; len(fields) > 0 {
		...
		if gq.withOwnerUser != nil {
			_spec.Node.AddColumnOnce(group.FieldUserID)
		}
	}
